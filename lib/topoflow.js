...
var Flow = function () {
    ...
    function Flow(config) {
        ...
        //初始化force
        var then = this;
        this.force = d3.forceSimulation(Object.values(this.Nodes)).alphaDecay(config.alphaDecay || 0.05) // 设置alpha衰减系数
        .force("link", d3.forceLink(Object.values(this.Links)).id(function (d) {
            return d.id;
        }).distance(config.distance || 5)) // distance为连线的距离设置
        .force('collide', d3.forceCollide().radius(function () {
            return config.radius || 50;
        ...
        .on('tick', function () {
            Object.values(_this.Nodes).forEach(function (node) {
                d3.select('#' + node.domId).attr('transform', function () {
    ...
    _createClass(Flow, [{
            ...
            if (!!!this.config.readOnly) {
                ...
                this.DragLinkEvent = d3.drag().on('start', function (d) {
                    ...
                    if (flag === 2) {
                        ...
                        var nodeID = d3.event.sourceEvent.target.parentNode.attributes.bid.nodeValue;
        ...
        value: function nodaDrag() {
            ...
            this.dragEvent = function () {
                return d3.drag().on('start', function (d) {
                    ...
                    // 移动节点,线条跟着变化
                    var nodeID = this.attributes.bid.nodeValue;
        ...
        value: function addNode(nodeInfo) {
            var then = this;
            var uuid = _common2.default.genUUID();
            ...
            var node = this.nodeGroup.append('g').attr('class', 'node').attr('id', nodeInfo.domId).attr('bid', nodeInfo.id).on('contextmenu', function () {
            ...
            then.force.nodes(Object.values(this.Nodes)).force("link", d3.forceLink(Object.values(then.Links)).id(function (d) {
                return d.id;
            }).distance(then.config.distance || 40)); // distance为连线的距离设置
        ...
        value: function onNodeClick(node, nodeInfo) {
            ...
            this.optionGroup = this.nodeGroup.append('g').attr('id', 'edit-' + nodeInfo.domId);
            if (!!!this.config.readOnly) {
